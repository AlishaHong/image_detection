...
def apply_random_transformations(image, bboxes, category_ids):
    ...
    # brightness_limit = random.uniform(0.2, 0.7)  # 밝기 범위 설정  -> 기본값 범위로 하기로함
    # contrast_limit = random.uniform(0.2, 0.7)  # 대비 범위 설정   -> 기본값 범위로 하기로함
...
def process_images_in_folder_random(folder_path, output_folder, num_variations=600):
    """
...
# 예제 사용
input_folder = 'snack_dataOrg'  # 원본 이미지와 TXT 파일이 있는 폴더
output_folder = 'snack_dataAugmentation_random'  # 변환된 이미지와 TXT 파일을 저장할 폴더

if not os.path.exists(output_folder):
    os.makedirs(output_folder)

process_images_in_folder_random(input_folder, output_folder, num_variations=600)
